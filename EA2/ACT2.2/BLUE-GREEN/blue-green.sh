...
SERVICE_NAME=$1        # Nombre del Service (ej: duoc-app-bg-service)
DEPLOYMENT_GREEN_NAME=$2 # Nombre del nuevo Deployment (ej: duoc-app-green)
YAML_FILE_GREEN=$3     # Ruta al archivo YAML del Deployment Green
...
# 3. SWITCH DE TRÁFICO (ROLLOUT ACTIVO)
...
# 4. VERIFICACIÓN DE CONTINUIDAD Y DISPONIBILIDAD EXTERNA
echo "[4] Verificando respuesta 200 OK y confirmando el contenido de $TARGET_VERSION_COLOR..."
...
while ! curl -s "http://$LB_URL" | grep -q "$TARGET_VERSION_COLOR"; do
    ...
    # o que haya un breve instante de indisponibilidad.
    ...
    if [ "$RESPONSE_CODE" != "200" ]; then
        ...
        # Para B/G, si hay un fallo aquí, significa que la estrategia falló.
    fi
    sleep 0.1
done
...
echo "B. Velocidad de Switch (kubectl patch): $SWITCH_DURATION segundos"
echo "C. Tiempo de Propagación (Patch hasta 200 OK/Green): $PROPAGATION_DURATION segundos"
echo "D. Tiempo TOTAL E2E (Apply -> Green OK): $TOTAL_DURATION segundos"
echo "E. Uptime / Downtime: Cero Interrupción (El servicio Blue estuvo activo en todo momento)"
echo "F. Tiempo de Provisionamiento del LB (Solo 1ra vez): $LB_PROVISIONING_DURATION segundos"